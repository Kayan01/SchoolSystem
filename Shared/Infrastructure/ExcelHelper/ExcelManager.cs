...
namespace ExcelManager
{
    public class ExcelReader
    {
        ...
        /// <returns></returns>
        public List<T> ReadAllSheets<T>(bool includeHeaders = false, Func<List<ICell>, string, object> predicate = null)
        {
            ...
            _readableSheets.ForEach(sheet =>
            {
              ...
              .Select(p => p).ToList();
                    ...
                    if (currentCells.Count > 0)
                    {
                        ...
                        var rowIndex = instance.GetType().GetProperties().Where(x => x.Name == "RowIndex").FirstOrDefault();
                        if (rowIndex != null)
                            rowIndex.SetValue(instance, row.RowNum);
