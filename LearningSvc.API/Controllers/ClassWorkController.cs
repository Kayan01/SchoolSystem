...
namespace LearningSvc.API.Controllers
{
    ...
    public class ClassWorkController : BaseController
    {
        ...
        public async Task<IActionResult> GetAllFileByTeacher([FromQuery] long teacherId, [FromQuery] int pagenumber, [FromQuery] int pagesize)
        {
            try
            {
                var result = await _classworkService.GetAllFileByTeacher(teacherId, pagenumber, pagesize);
                if (result.HasError)
        ...
        public async Task<IActionResult> GetAllFileByClass([FromQuery] long classId, [FromQuery] int pagenumber, [FromQuery] int pagesize)
        {
            try
            {
                var result = await _classworkService.GetAllFileByClass(classId, pagenumber, pagesize);
                if (result.HasError)
                    return ApiResponse<object>(errors: result.ErrorMessages.ToArray());
                return ApiResponse<object>(message: "Successful", codes: ApiResponseCodes.OK, data: result.Data);
            }
        ...
        [HttpPost]
        [ProducesResponseType(typeof(ApiResponse<object>), 200)]
        public async Task<IActionResult> UploadFile(ClassWorkUploadVM model)
        {
            if (model == null)
        ...
        public async Task<IActionResult> DeleteFile(long id)
        {
            ...
            try
            {
                var result = await _classworkService.DeleteLearningFile(id);
